...
CREATE OR REPLACE FUNCTION "public"."compare_and_update_set_product_stock"("owner_id_input" "text", "product_id_input" "text") RETURNS "public"."d_products"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
    ...
    IF set_product."isSetArticle" = false OR jsonb_array_length(set_product."listOfProductIdWithQuantity") = 0 THEN
    RAISE EXCEPTION 'Der Artikel ist kein Set-Artikel oder die Liste der Set-Teilprodukte ist leer';
    END IF;

    -- Liste der Set-Teil-Produkte abrufen
    listOfSetPartProducts := ARRAY(SELECT jsonb_array_elements(set_product."listOfProductIdWithQuantity"));

    -- Verfügbarkeit des Set-Artikels berechnen
    available_quantity_set_article := (
        SELECT MIN((sp."availableStock" / (part_product->>'quantity')::INT))
        FROM jsonb_array_elements(set_product."listOfProductIdWithQuantity") AS part_product,
             d_products sp
        WHERE sp.id = (part_product->>'productId')::TEXT
